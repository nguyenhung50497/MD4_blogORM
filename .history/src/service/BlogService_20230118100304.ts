...
class BlogService {
    ...
    getAll = async () => {
        let sql = `SELECT * FROM Blog p JOIN BlogTag b ON p.id = b.blog JOIN tag t ON b.tag = tag.idTag`
    ...
    findById = async (id) => {
        let sql = `SELECT * FROM Blog p JOIN BlogTag b ON p.id = b.blog JOIN tag t ON b.tag = tag.idTag WHERE id = '%${id}%'`
        let blog = await this.blogRepository.query({id: id});
        if (!blog) {
            return null;
        }
        return blog;
