...
class BlogService {
    ...
    getMyBlog = async(idUser) => {
        let sql = `SELECT * FROM user u JOIN blog p ON u.idUser = p.user JOIN blog_tag b ON p.id = b.blog JOIN tag t ON b.tag = t.idTag WHERE idUser = ${idUser} AND statusBlog = ''`
        let blogs = await this.blogRepository.query(sql);
        return blogs;
    }
