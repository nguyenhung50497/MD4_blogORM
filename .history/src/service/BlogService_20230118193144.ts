...
class BlogService {
    ...
    search = async (name, idUser) => {
        ...
        sql = `SELECT * FROM blog p JOIN blog_tag b ON p.id = b.blog JOIN tag t ON b.tag = t.idTag JOIN user u  WHERE name LIKE '%${name}%' AND statusBlog = 'Private' AND idUser = ${idUser}`;
