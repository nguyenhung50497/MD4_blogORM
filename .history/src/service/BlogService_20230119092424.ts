...
class BlogService {
    ...
    search = async (name, idUser) => {
        let sql = `SELECT * FROM blog p JOIN blog_tag b ON p.id = b.blog JOIN tag t ON b.tag = t.idTag JOIN user u ON p.user = u.idUser WHERE name LIKE '%${name}%' AND statusBlog = 'Public'`;
        let blogs1 = await this.blogRepository.query(sql);
        sql = `SELECT * FROM blog p JOIN blog_tag b ON p.id = b.blog JOIN tag t ON b.tag = t.idTag JOIN user u ON p.user = u.idUser WHERE name LIKE '%${name}%' AND statusBlog = 'Private' AND idUser = ${idUser}`;
    ...
    searchByAdmin = async (name) => {
        let sql = `SELECT * FROM blog p JOIN blog_tag b ON p.id = b.blog JOIN tag t ON b.tag = t.idTag JOIN user u ON p.user = u.idUser WHERE name LIKE '%${name}%'`;
        let blogs = await this.blogRepository.query(sql);
        if (!blogs) {
            return null;
        }
