...
class BlogService {
    ...
    search = async (name, idUser) => {
        ...
        sql = `SELECT * FROM blog p JOIN blog_tag b ON p.id = b.blog JOIN tag t ON b.tag = t.idTag WHERE name LIKE '%${name}%' AND statusBlog = 'Private' AND idUser = %${idUser}`;
        let blogs2 = await this.blogRepository.query(sql);
        let blogs = await blogs1.concat(blogs
        if (!blogs1) {
            return null;
        }
        return blogs1;
    ...
    searchByAdmin = async (name, idUser) => {
